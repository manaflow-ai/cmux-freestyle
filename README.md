...
`skills` installs the `cmux-freestyle` agent skill from `skills/cmux-freestyle/SKILL.md` into another project's `.agents/skills/cmux-freestyle/` and `.claude/skills/cmux-freestyle/` (the cross-agent convention shared by Claude Code, Codex, OpenCode, Amp, Goose, Gemini CLI). The skill itself is auto-wired inside this checkout via tracked relative symlinks (`.claude/skills/cmux-freestyle/` and `.agents/skills/`), so any AI agent working *in* `cmux-freestyle/` already sees it; you only need `./setup.sh skills install --target <other-project>` if you want the skill available in a different repo. Default mode is `--link` (symlink to this checkout so `git pull` auto-upgrades); pass `--copy` for a portable copy. `uninstall`, `doctor`, `list`, `--check` are also there. `./skills.sh` works on its own too.
